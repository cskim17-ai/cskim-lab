...
export default function AdminInteractiveMap() {
  ...
  return (
    <div className="space-y-8 pb-32 px-4 sm:px-6 md:px-0">
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-white/5 pb-10">
        <div className="space-y-4 text-center lg:text-left">
          <div className="flex flex-col items-center lg:items-start gap-4">
             <div className="w-14 h-14 bg-lime/10 rounded-2xl flex items-center justify-center text-lime shadow-xl">
               <MapIcon size={28} />
             </div>
             <h2 className="text-4xl sm:text-6xl font-black italic serif text-white tracking-tighter">인터랙티브 지도</h2>
          </div>
          <p className="text-[12px] text-white/40 uppercase tracking-[0.5em] font-black">위치 인텔리전스 v1.0</p>
        </div>
        
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          {/* Search Form */}
          <form onSubmit={handleSearch} className="relative w-full sm:min-w-[320px]">
            <input
              type="text"
              placeholder="장소, 주소 검색..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 pl-14 text-sm text-white focus:outline-none focus:border-lime/50 transition-all placeholder:text-white/20"
            />
            <Search size={20} className="absolute left-5 top-1/2 -translate-y-1/2 text-white/20" />
            <button 
              type="submit"
              disabled={isSearching || !searchQuery.trim()}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-2.5 text-lime hover:bg-lime/10 rounded-xl transition-all disabled:opacity-30 active:scale-90"
            >
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-3 px-8 py-4 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-2xl font-black text-xs uppercase tracking-widest transition-all border border-red-500/20 disabled:opacity-30 disabled:cursor-not-allowed shrink-0 active:scale-95"
          >
            <Trash2 size={18} />
            전체 지우기
          </button>
