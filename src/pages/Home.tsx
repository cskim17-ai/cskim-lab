...
export default function Home() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(db, 'settings', 'logo'), (snap) => {
      ...
      // Handle permission denied for public logo
      if (error.code !== 'permission-denied') {
        console.error("Home logo error:", error);
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-forest text-white flex flex-col">
      ...
      <header className="border-b border-white/10 px-6 py-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">cskim-lab</h1>
      ...
      <main className="flex-1 flex items-center justify-center px-6 py-20">
        ...
        >
          <h2 className="text-5xl font-bold mb-6">
            <span className="text-lime">cskim-lab</span> 관리 시스템
          </h2>
      ...
      <footer className="border-t border-white/10 px-6 py-6 text-center text-white/40 text-sm">
        <p>© 2024 cskim-lab. All rights reserved.</p>
      </footer>
    </div>
  );
}
